...
export default function useFilter(options: UseFilterOptions) {
		...
		return yup.object().shape<FilterState>({
			...
			order: yup.object().shape({
				sort: yup
					...
					.nullable()
					.transform((value) => {
						const columnsName = columns.filter((column) => !column.options || column.options.sort !== false).map((column) => column.name);
	...
	}, [history, formatSearchParams, cleanSearchText, debouncedFilterState, extraFilter, locationPathname, locationState]);
