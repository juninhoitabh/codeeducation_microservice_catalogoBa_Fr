...
const UploadField = React.forwardRef<UploadFieldComponent, UploadFieldProps>((props, ref) => {
	...
	return (
		<FormControl error={error !== undefined} disabled={disabled === true} fullWidth margin={'normal'} {...props.FormControlProps}>
			<InputFile
				ref={fileRef}
				TextFieldProps={{
					label: label,
				...
				ButtonFile={
					<Button endIcon={<CloudUploadIcon />} variant={'contained'} color={'primary'} onClick={() => fileRef.current.openWindow()}>
						Adicionar
					</Button>
				}
			/>
